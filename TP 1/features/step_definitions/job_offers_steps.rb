...
When(/^I create a new offer with "(.*?)" as the title$/) do |title|
  ...
  fill_in('job_offer_form[remuneration]', with: '10')
  click_button('Create')
end

When('I create a new offer with remuneration {string}') do |remuneration|
  visit '/job_offers/new'
  fill_in('job_offer_form[title]', with: 'title')
...
Then('I should see {string} as remuneration') do |remuneration|
  visit '/job_offers/my'
